...
func TestExamplesComplete(t *testing.T) {
	...
	testStructure.RunTestStage(t, "apply", func() {
		...
		terraformOptions.TerraformDir = tempTestFolder + "/images"

		// Run `terraform output` to get the value of an output variable
		repositories := terraform.Output(t, terraformOptions, "repositories")

		// Verify we're getting back the outputs we expect
		assert.NotEmpty(t, repositories, "Expected `repositories` to be populated")
	})
}
...
func TestExamplesCompleteDisabled(t *testing.T) {
	...
	rootFolder := "../../"
	terraformFolderRelativeToRoot := "examples/complete/images"
